...
\Address{
  ...
  URL: \url{http://www.wwz.unibas.ch/qm/team/kleiber/kleiber.htm}\\
...
The full \proglang{R} code from the book is provided in the demos for the package \pkg{AER}. The source scripts can be found in the \code{demo}
...
in the daily checks, a few selected code chunks are commented out in the scripts. Also, for technical reasons, some graphics code chunks are repeated, once commented out and once without comments.
